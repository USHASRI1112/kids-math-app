...
function createMultipleChoiceQuestion(index: number): QuizQuestion {
  const { left, right, answer } = generateAdditionPair();
  const options = shuffle([answer, ...uniqueNumbers(3, 1, 20, answer)]);
...
function createMissingNumberQuestion(index: number): QuizQuestion {
  ...
  return {
    ...
    type: 'missing_number',
    prompt: missingSide === 'left' ? `__ + ${right} = ${answer}` : `${left} + __ = ${answer}`,
    questionText: missingSide === 'left' ? `__ + ${right} = ${answer}` : `${left} + __ = ${answer}`,
...
export default function AdditionQuizModule() {
  ...
  function commitAnswer(userAnswerText: string, isCorrect: boolean) {
    ...
    setLocked(true);
    setFeedback(isCorrect ? t('addition_quiz.correct', { defaultValue: 'Correct!' }) : t('addition_quiz.try_again', { defaultValue: 'Try Again' }));
  ...
  function goNextQuestion() {
    if (currentIndex + 1 >= questions.length) {
      ...
      if (finalScore === QUIZ_SIZE) {
        setBadge(t('addition_quiz.badge_master', { defaultValue: 'Addition Quiz Master' }));
      }
      return;
    }
    ...
    <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
      <View style={styles.headerCard}>
        <View>
          ...
          <Text style={styles.headerTitle}>{t('addition_quiz.title', { defaultValue: 'Addition Quiz' })}</Text>
          <Text style={styles.headerSubtitle}>{t('addition_quiz.subtitle', { defaultValue: 'Mixed quiz mode' })}</Text>
        </View>
      </View>
      ...
      {!completed ? (
        <>
          <View style={styles.progressCard}>
            <Text style={styles.progressLabel}>
              {t('addition_quiz.progress', { defaultValue: 'Question {{current}} / {{total}}', current: progress, total: QUIZ_SIZE })}
            </Text>
          ...
          <View style={styles.quizCard}>
            ...
            <Text style={styles.hintText}>{feedback ?? t('addition_quiz.choose_answer', { defaultValue: 'Choose an answer.' })}</Text>
            ...
            {currentQuestion?.type === 'fill_blank' || currentQuestion?.type === 'missing_number' ? (
              <View style={styles.numericSection}>
                ...
                <Pressable style={styles.primaryButton} onPress={submitNumericAnswer} accessibilityRole="button">
                  <Text style={styles.primaryButtonText}>{t('addition_quiz.submit', { defaultValue: 'Check Answer' })}</Text>
                </Pressable>
              </View>
            ) : null}
            ...
            {currentQuestion?.type === 'true_false' ? (
              <View style={styles.trueFalseRow}>
                <Pressable
                  ...
                  accessibilityRole="button"
                >
                  <Text style={styles.trueFalseText}>{t('addition_quiz.true', { defaultValue: 'TRUE' })}</Text>
                </Pressable>
                <Pressable
                  style={[
                    styles.trueFalseButton,
                  ...
                  accessibilityRole="button"
                >
                  <Text style={styles.trueFalseText}>{t('addition_quiz.false', { defaultValue: 'FALSE' })}</Text>
                </Pressable>
              </View>
            ) : null}
                ...
                <Pressable style={styles.primaryButton} onPress={goNextQuestion} accessibilityRole="button">
                  <Text style={styles.primaryButtonText}>
                    {currentIndex + 1 >= questions.length
                      ? t('addition_quiz.results', { defaultValue: 'See Results' })
                      : t('addition_quiz.next_question', { defaultValue: 'Next Question' })}
                  </Text>
                </Pressable>
              </View>
            ) : null}
        ...
        <View style={styles.resultsCard}>
          <Text style={styles.completeTitle}>{t('addition_quiz.quiz_complete', { defaultValue: 'Quiz Complete' })}</Text>
          <Text style={styles.completeBody}>{rating.label}</Text>

          <View style={styles.statsGrid}>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{score}/{QUIZ_SIZE}</Text>
              <Text style={styles.statLabel}>{t('addition_quiz.score', { defaultValue: 'Score' })}</Text>
            </View>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{rating.accuracy}%</Text>
              <Text style={styles.statLabel}>{t('addition_quiz.accuracy', { defaultValue: 'Accuracy' })}</Text>
            </View>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{correctCount}</Text>
              <Text style={styles.statLabel}>{t('addition_quiz.correct_answers', { defaultValue: 'Correct Answers' })}</Text>
            </View>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{incorrectCount}</Text>
              <Text style={styles.statLabel}>{t('addition_quiz.incorrect_answers', { defaultValue: 'Incorrect Answers' })}</Text>
            </View>
          </View>

          <View style={styles.rewardCard}>
            <Text style={styles.rewardLabel}>{t('addition_quiz.rewards', { defaultValue: 'Rewards' })}</Text>
          ...
          <View style={styles.reviewHeader}>
            <Text style={styles.reviewTitle}>{t('addition_quiz.review_title', { defaultValue: 'Review Answers' })}</Text>
            <Pressable style={styles.secondaryButton} onPress={resetQuiz} accessibilityRole="button">
              <Text style={styles.secondaryButtonText}>{t('addition_quiz.replay', { defaultValue: 'Replay Quiz' })}</Text>
            </Pressable>
          </View>
              ...
              <View key={item.id} style={styles.reviewItem}>
                ...
                {!item.isCorrect ? (
                  <Text style={styles.reviewCorrectAnswer}>
                    {t('addition_quiz.correct_answer', { defaultValue: 'Correct Answer' })}: {item.correctAnswerText}
                  </Text>
                ) : null}
              </View>
            ))}
