...
const STORAGE_PREFIX = 'addition-timer-best';
...
function feedbackMessage(t: (key: string, options?: Record<string, unknown>) => string, correct: boolean) {
  return correct
    ? t('addition_timer.correct', { defaultValue: 'Correct!' })
    : t('addition_timer.incorrect', { defaultValue: 'Try Again!' });
}

function speedMessage(score: number, isNewHighScore: boolean) {
  if (isNewHighScore) {
    return 'Beat Your Best Score!';
  }
  if (score >= 25) {
...
export default function AdditionTimerModule() {
  ...
  const durationLabel = useMemo(() => {
    if (selectedDuration === 30) return t('addition_timer.duration_30', { defaultValue: '30 Seconds' });
    if (selectedDuration === 60) return t('addition_timer.duration_60', { defaultValue: '60 Seconds' });
    if (selectedDuration === 120) return t('addition_timer.duration_120', { defaultValue: '120 Seconds' });
    return '';
  }, [selectedDuration, t]);
    ...
    <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.headerCard}>
        <View>
          ...
          <Text style={styles.headerTitle}>{t('addition_timer.title', { defaultValue: 'Addition Timer' })}</Text>
          <Text style={styles.headerSubtitle}>{t('addition_timer.subtitle', { defaultValue: 'Speed Challenge' })}</Text>
        </View>
      </View>

      {!durationLoaded ? (
        <View style={styles.card}>
          <Text style={styles.cardBody}>{t('addition_timer.loading', { defaultValue: 'Loading...' })}</Text>
        </View>
      ) : null}

      {!running && !completedResult ? (
        <View style={styles.card}>
          <Text style={styles.sectionTitle}>{t('addition_timer.choose_duration', { defaultValue: 'Select Duration' })}</Text>
              ...
              return (
                ...
                >
                  <Text style={[styles.durationText, active && styles.durationTextActive]}>
                    {duration} {t('addition_timer.seconds', { defaultValue: 'Seconds' })}
                  </Text>
                  <Text style={[styles.durationScore, active && styles.durationTextActive]}>
                    {t('addition_timer.best', { defaultValue: 'Best' })}: {highScore}
                  </Text>
                </Pressable>
              );
            })}
        ...
        <View style={styles.card}>
          <View style={styles.statsRow}>
            <View style={styles.statPill}>
              <Text style={styles.statLabel}>{t('addition_timer.time_left', { defaultValue: 'Time Left' })}</Text>
              <Text style={styles.statValue}>{timeLeft}s</Text>
            </View>
            <View style={styles.statPill}>
              <Text style={styles.statLabel}>{t('addition_timer.score', { defaultValue: 'Score' })}</Text>
              <Text style={styles.statValue}>{score}</Text>
            </View>
            <View style={styles.statPill}>
              <Text style={styles.statLabel}>{t('addition_timer.best', { defaultValue: 'Best' })}</Text>
          ...
          <Text style={styles.feedbackText}>{feedback ?? t('addition_timer.answer_fast', { defaultValue: 'Answer fast!' })}</Text>
      ...
      {completedResult ? (
        <View style={styles.resultsCard}>
          <Text style={styles.completeTitle}>{t('addition_timer.time_up', { defaultValue: 'Time Up!' })}</Text>
          ...
          <View style={styles.statsGrid}>
            ...
            <View style={styles.statCard}>
              ...
              <Text style={styles.statLabel}>{t('addition_timer.questions_attempted', { defaultValue: 'Questions Attempted' })}</Text>
            </View>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{completedResult.correct}</Text>
              <Text style={styles.statLabel}>{t('addition_timer.correct_answers', { defaultValue: 'Correct Answers' })}</Text>
            </View>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{completedResult.incorrect}</Text>
              <Text style={styles.statLabel}>{t('addition_timer.incorrect_answers', { defaultValue: 'Incorrect Answers' })}</Text>
            </View>
          </View>

          <View style={styles.resultsSummary}>
            <Text style={styles.summaryText}>{t('addition_timer.accuracy', { defaultValue: 'Accuracy' })}: {completedResult.accuracy}%</Text>
            <Text style={styles.summaryText}>
              {durationLabel} {t('addition_timer.best', { defaultValue: 'Best' })}: {completedResult.newBest}
            </Text>
            {completedResult.isNewHighScore ? (
              <Text style={styles.highScore}>{t('addition_timer.new_high_score', { defaultValue: 'New High Score!' })}</Text>
            ) : null}
          </View>

          <View style={styles.actionRow}>
            <Pressable style={styles.secondaryButton} onPress={replay} accessibilityRole="button">
              <Text style={styles.secondaryButtonText}>{t('addition_timer.play_again', { defaultValue: 'Play Again' })}</Text>
            </Pressable>
            <Pressable style={styles.primaryButton} onPress={() => setCompletedResult(null)} accessibilityRole="button">
              <Text style={styles.primaryButtonText}>{t('addition_timer.change_duration', { defaultValue: 'Change Duration' })}</Text>
            </Pressable>
          </View>
        </View>
      ) : null}
