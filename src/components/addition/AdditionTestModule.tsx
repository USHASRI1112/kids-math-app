...
const STORAGE_KEY = 'addition-test-state';
...
function uniqueNumbers(count: number, exclude: number) {
  ...
  while (values.size < count) {
    const candidate = randomInt(1, 20);
...
function createMissingNumber(index: number): TestQuestion {
  ...
  return {
    ...
    left,
    right,
    answer: missingLeft ? left : right,
    correctText: String(missingLeft ? left : right),
  };
}
...
function getFalseResult(correct: number) {
  ...
  return pool.length ? pool[randomInt(0, pool.length - 1)] : Math.min(20, correct + 1);
}
...
function badgeForScore(accuracy: number, score: number) {
  if (score === TEST_QUESTION_COUNT) {
    return 'Addition Champion Badge';
  }
  if (accuracy >= 90) {
    return 'Addition Master Badge';
  }
  if (accuracy >= 60) {
    return 'Addition Explorer Badge';
  }
  return null;
}
...
export default function AdditionTestModule() {
  ...
  const progress = `${currentIndex + 1} / ${TEST_QUESTION_COUNT}`;
  const timeDisplay = formatSeconds(timeLeft);
  ...
  function recordAnswer(questionIndex: number, selected: string) {
    ...
    setFeedback(t('addition_test.saved', { defaultValue: 'Saved' }));
    setInputValue('');
  }
    ...
    <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.headerCard}>
        <View>
          ...
          <Text style={styles.headerTitle}>{t('addition_test.title', { defaultValue: 'Addition Test' })}</Text>
          <Text style={styles.headerSubtitle}>{t('addition_test.subtitle', { defaultValue: '15 minute assessment' })}</Text>
        </View>
      </View>

      {mode === 'instructions' ? (
        <View style={styles.card}>
          <Text style={styles.sectionTitle}>{t('addition_test.instructions_title', { defaultValue: 'Instructions' })}</Text>
          <Text style={styles.bodyText}>{t('addition_test.instructions_body', { defaultValue: 'Answer 20 mixed addition questions. Take your time, review unanswered items, and submit when you are done.' })}</Text>
          <View style={styles.instructionsList}>
            <Text style={styles.bulletText}>• {t('addition_test.instructions_time', { defaultValue: 'You have 15 minutes.' })}</Text>
            <Text style={styles.bulletText}>• {t('addition_test.instructions_review', { defaultValue: 'You can move back and forth between questions.' })}</Text>
            <Text style={styles.bulletText}>• {t('addition_test.instructions_auto', { defaultValue: 'Your answers are saved automatically.' })}</Text>
          </View>
          <Pressable style={styles.primaryButton} onPress={startNewTest} accessibilityRole="button">
            <Text style={styles.primaryButtonText}>{t('addition_test.start_test', { defaultValue: 'Start Test' })}</Text>
          </Pressable>
        </View>
      ) : null}
        ...
        <>
          <View style={styles.topStatsCard}>
            <View style={styles.topStat}>
              <Text style={styles.topStatLabel}>{t('addition_test.time_remaining', { defaultValue: 'Time Remaining' })}</Text>
              <Text style={styles.topStatValue}>{timeDisplay}</Text>
            </View>
            <View style={styles.topStat}>
              <Text style={styles.topStatLabel}>{t('addition_test.score', { defaultValue: 'Score' })}</Text>
              <Text style={styles.topStatValue}>{score}</Text>
            </View>
            <View style={styles.topStat}>
              <Text style={styles.topStatLabel}>{t('addition_test.answered', { defaultValue: 'Answered' })}</Text>
          ...
          <View style={styles.card}>
            <View style={styles.progressHeader}>
              <Text style={styles.sectionTitle}>{t('addition_test.question_of', { defaultValue: 'Question {{current}} of {{total}}', current: currentIndex + 1, total: TEST_QUESTION_COUNT })}</Text>
              <Pressable style={styles.secondaryButton} onPress={showQuestionNavigator} accessibilityRole="button">
                <Text style={styles.secondaryButtonText}>
                  {reviewMode ? t('addition_test.hide_review', { defaultValue: 'Hide Review' }) : t('addition_test.review_unanswered', { defaultValue: 'Review Unanswered' })}
                </Text>
              </Pressable>
            </View>
            ...
            <Text style={styles.progressMeta}>
              {t('addition_test.unanswered', { defaultValue: 'Unanswered' })}: {unansweredCount}
            </Text>
            ...
            <Text style={styles.feedbackText}>
              {feedback ?? t('addition_test.answer_prompt', { defaultValue: 'Take your time and answer carefully.' })}
            </Text>
              ...
              <View style={styles.numericSection}>
                ...
                {(currentQuestion?.type === 'fill_blank' || currentQuestion?.type === 'missing_number') && currentSelected ? (
                  <View style={styles.savedAnswerPill}>
                    <Text style={styles.savedAnswerText}>
                      {t('addition_test.saved_answer', { defaultValue: 'Saved Answer' })}: {currentSelected}
                    </Text>
                  </View>
                ) : null}
                ...
                <Pressable style={styles.primaryButton} onPress={submitNumericAnswer} accessibilityRole="button">
                  <Text style={styles.primaryButtonText}>
                    {currentQuestion?.type === 'fill_blank' || currentQuestion?.type === 'missing_number'
                      ? t('addition_test.save_answer', { defaultValue: 'Save Answer' })
                      : t('addition_test.submit_answer', { defaultValue: 'Submit Answer' })}
                  </Text>
                </Pressable>
              </View>
            ) : null}
            ...
            {currentQuestion?.type === 'true_false' ? (
              <View style={styles.trueFalseRow}>
                <Pressable
                  ...
                  accessibilityRole="button"
                >
                  <Text style={styles.trueFalseText}>{t('addition_test.true', { defaultValue: 'TRUE' })}</Text>
                </Pressable>
                <Pressable
                  style={[
                    styles.trueFalseButton,
                  ...
                  accessibilityRole="button"
                >
                  <Text style={styles.trueFalseText}>{t('addition_test.false', { defaultValue: 'FALSE' })}</Text>
                </Pressable>
              </View>
            ) : null}
            ...
            <View style={styles.actionRow}>
              ...
              >
                <Text style={styles.secondaryButtonText}>{t('addition_test.previous', { defaultValue: 'Previous' })}</Text>
              </Pressable>
              <Pressable
              ...
              >
                <Text style={styles.secondaryButtonText}>{t('addition_test.next', { defaultValue: 'Next' })}</Text>
              </Pressable>
              <Pressable style={styles.primaryButton} onPress={submitTest} accessibilityRole="button">
                <Text style={styles.primaryButtonText}>{t('addition_test.submit_test', { defaultValue: 'Submit Test' })}</Text>
              </Pressable>
            </View>
            <Pressable style={styles.jumpButton} onPress={jumpToFirstUnanswered} accessibilityRole="button">
              <Text style={styles.jumpButtonText}>{t('addition_test.jump_unanswered', { defaultValue: 'Jump to First Unanswered' })}</Text>
            </Pressable>
          </View>
        </>
      ) : null}
      ...
      {mode === 'results' && results ? (
        <View style={styles.resultsCard}>
          <Text style={styles.completeTitle}>{t('addition_test.complete', { defaultValue: 'Addition Test Complete' })}</Text>
          <Text style={styles.completeBody}>
            {t('addition_test.score_label', { defaultValue: 'Score' })}: {results.score} / {TEST_QUESTION_COUNT}
          </Text>

          <View style={styles.statsGrid}>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{results.correct}</Text>
              <Text style={styles.statLabel}>{t('addition_test.correct_answers', { defaultValue: 'Correct Answers' })}</Text>
            </View>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{results.incorrect}</Text>
              <Text style={styles.statLabel}>{t('addition_test.incorrect_answers', { defaultValue: 'Incorrect Answers' })}</Text>
            </View>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{results.accuracy}%</Text>
              <Text style={styles.statLabel}>{t('addition_test.accuracy', { defaultValue: 'Accuracy' })}</Text>
            </View>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{formatSeconds(results.timeUsed)}</Text>
              <Text style={styles.statLabel}>{t('addition_test.time_used', { defaultValue: 'Time Used' })}</Text>
            </View>
          </View>

          <View style={styles.summaryCard}>
            <Text style={styles.summaryLabel}>{t('addition_test.grade', { defaultValue: 'Grade' })}</Text>
            <Text style={styles.summaryValue}>{results.grade}</Text>
            <Text style={styles.summaryBody}>{results.unanswered > 0 ? t('addition_test.unanswered_remaining', { defaultValue: 'Some questions were left unanswered.' }) : t('addition_test.all_answered', { defaultValue: 'All questions were answered.' })}</Text>
            <Text style={styles.summaryBody}>{t('addition_test.strong_areas', { defaultValue: 'Strong Areas' })}: {results.strongArea}</Text>
            <Text style={styles.summaryBody}>{t('addition_test.needs_improvement', { defaultValue: 'Needs Improvement' })}: {results.needsImprovement}</Text>
            <Text style={styles.summaryBody}>{t('addition_test.recommended_practice', { defaultValue: 'Recommended Practice Activities' })}: {t('activities.practice')}</Text>
            {results.badge ? <Text style={styles.badgeText}>{results.badge}</Text> : null}
          </View>

          <View style={styles.reviewHeader}>
            <Text style={styles.reviewTitle}>{t('addition_test.review_answers', { defaultValue: 'Review Answers' })}</Text>
          </View>
            ...
            {reviewItems.map((item, index) => (
              <View key={`${item.questionText}-${index}`} style={styles.reviewItem}>
                ...
                <Text style={[styles.reviewOutcome, item.isCorrect ? styles.reviewOutcomeCorrect : styles.reviewOutcomeWrong]}>
                  {item.selectedAnswer || t('addition_test.unanswered', { defaultValue: 'Unanswered' })} {item.isCorrect ? '✅' : '❌'}
                </Text>
                <Text style={styles.reviewCorrect}>
                  {t('addition_test.correct_answer', { defaultValue: 'Correct Answer' })}: {item.correctAnswer}
                </Text>
              </View>
            ))}
          </View>
          ...
          <View style={styles.actionRow}>
            <Pressable style={styles.primaryButton} onPress={startNewTest} accessibilityRole="button">
              <Text style={styles.primaryButtonText}>{t('addition_test.retry', { defaultValue: 'New Test' })}</Text>
            </Pressable>
          </View>
        </View>
      ) : null}
